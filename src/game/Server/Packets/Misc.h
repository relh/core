...
namespace WorldPackets { namespace ClassDeck
{
    ...
    public:
        ...
        explicit Hello() : ClientPacket(CMSG_COWORLD_CLASS_DECK_HELLO) {}
        void ReadFromWorldPacket(WorldPacket& recv_data) override;
    };

    class Choose final : public ClientPacket
    {
    public:
        uint8 protocolVersion = 0;
        uint32 draftIndex = 0;
        uint32 serviceSpell = 0;
        std::string catalogVersion;

        explicit Choose() : ClientPacket(CMSG_COWORLD_CLASS_DECK_CHOOSE) {}
        void ReadFromWorldPacket(WorldPacket& recv_data) override;
    };
}} // namespace WorldPackets::ClassDeck
