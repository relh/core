...
void WorldRunnable::operator()()
{
    ...
    while (!World::IsStopped())
    {
        ...
        {
            uint32 simulationRemaining = sWorld.ScaleSimulationDuration(diff);
            uint32 operationalWallDiff = diff;
            while (simulationRemaining > 0)
            {
                uint32 const simulationStep = std::min<uint32>(
                    simulationRemaining, WORLD_SLEEP_CONST);
                sWorld.Update(simulationStep, operationalWallDiff);
                operationalWallDiff = 0;
